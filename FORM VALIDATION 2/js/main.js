...
function validator(options) {
  ...
  function validate (inputElement, rule) {
    ...
    //Lay ra cac rule cua tung Selector (#fullname, #email, #password, # password_confirmed)
  ...
  //LAY ELEMENT CUA FORM need to VALIDATE
  var formElement = document.querySelector(options.form);
  if (formElement) {

    //Khi click summit form
    formElement.onsubmit = function (e) {
      ...
      //Lap qua tung rule & validate
        ...
        if (isFormValid) {
          //Summit with Javascript
          if (typeof options.onSummit === 'function') {
            var enableInputs = formElement.querySelectorAll('[name]:not([disabled])'); //select all attribute 'name' va trang thai ko disabled
...
validator.isRequired = function (selector, message) {
  return {
    selector: selector,
    test: function (value) {
        return value ? undefined : message || "Please enter this information";
    }
  }
}
...
validator.minLength = function (selector, min, message) {
  return {
    selector: selector,
    test: function (value) {
      return value.length >= min ? undefined : message || `Please enter minimum ${min} words`;
    }
  }
}
...
validator.isConfirmed = function (selector, getConfirmValue, message) {
  return  {
    selector: selector,
    test: function (value) {
      return value === getConfirmValue() ? undefined : message || "Incorrect value confirmation";
    }
  }
}
